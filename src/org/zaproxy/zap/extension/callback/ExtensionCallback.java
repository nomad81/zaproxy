...
public class ExtensionCallback extends ExtensionAdaptor implements
    ...
    private void restartServer() {
        ...
        port = proxyServer.startServer(this.getCallbackParam()
                .getLocalAddress(), this.getCallbackParam().getPort(), true);
        LOGGER.info("Started callback server on "
                + this.getCallbackParam().getLocalAddress() + ":" + port);
    }
        ...
        public boolean onHttpRequestSend(HttpMessage msg) {
            try {
                ...
                if (path.startsWith("/" + TEST_PREFIX)) {
                    String str = Constant.messages.getString("callback.test.msg", 
                            url, msg.getRequestHeader().getSenderAddress().toString());
                    if (View.isInitialised()) {
                ...
                LOGGER.error("No callback handler for URL : " + url);
